...
object Payment {
  ...
  val restartStrategy = SupervisorStrategy.restart.withLimit(maxNrOfRetries = 5, withinTimeRange = 1.second)
    ...
    Behaviors
      .receive[Message]((context, msg) =>
        msg match {
          case DoPayment =>
            val responseAdapter =
              context.messageAdapter[PaymentService.Response] { case response @ PaymentService.PaymentSucceeded =>
                WrappedPaymentServiceResponse(response)
              }
            val paymentService = Behaviors
              .supervise(PaymentService(method, responseAdapter))
              .onFailure(restartStrategy)
            //val paymentServiceRef = context.spawnAnonymous(PaymentService(method, responseAdapter))
      ...
      .receiveSignal { case (context, Terminated(t)) =>
