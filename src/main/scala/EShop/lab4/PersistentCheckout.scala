...
class PersistentCheckout {
  ...
  def schedule(context: ActorContext[Command], msg: Command): Cancellable =
    context.scheduleOnce(delay = timerDuration, target = context.self, msg = msg)
      ...
      case SelectingPaymentMethod(_) =>
        command match {
          case SelectPayment(payment, orderManagerRef) =>
            ...
            Effect.persist(PaymentStarted(paymentRef)).thenRun { _ =>
      ...
      case ProcessingPayment(_) =>
        command match {
          ...
          case CancelCheckout | ExpirePayment =>
            Effect.persist(CheckoutCancelled).thenRun { _ =>
